...
* assistance in the completion of this assignment. Signed: 22 03 2019 YOUR NAME JOOWON PARK
*
...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		case 5:
			...
			cout << "\tfunnyEars(9) = 22\n";
			while (1) {
				N = GetInt("\tEnter a number of bunnies and funnies(0 to quit): ");
				if (N <= 0) break;
		...
		case 6:
			...
			cout << "\ttriangle(7) = 28\n";
			while (1) {
				N = GetInt("\tEnter a number of triangle rows(0 to quit): ");
				if (N <= 0) break;
		...
		case 7:
			...
			while (1) {
				N = GetInt("\tEnter a number to compute sum of digits(0 to quit): ");
				if (N <= 0) break;
		...
		case 8:
			...
			cout << "\tcount8(99899) 		= 1\n";
			while (1) {
				N = GetInt("\tEnter a number to count 8s(0 to quit): ");
				if (N <= 0) break;
				cout << "\tcount8(" << N << ") = " << count8(N) << endl;
			}
			break;
		case 9:
			cout << "\tPowerN Examples:\n";
			cout << "\tpowerN(2, 5)  = 32\n";
			cout << "\tpowerN(3, 1)  = 3\n";
			cout << "\tpowerN(3, 2)  = 9\n";
			cout << "\tpowerN(3, 3)  = 27\n";
			...
			while (1) {
				...
				if (base == 0) break;
				int power = GetInt("\tEnter a power number(0 to quit): ");
				if (power == 0) break;
				cout << "\t" << base << " to the power of " << power << " is " << powerN(base, power) << endl;
			}
			break;
		...
		case 10:
			...
			while (1) {
	 		 ...
	 		 if(check == 0) cout << "\t" << key << " is not @["  << index << "]" << endl;
