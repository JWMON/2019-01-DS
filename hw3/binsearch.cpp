...
int _binary_search(int *data, int key, int lo, int hi) {
	...
	if(data[mid] > key) return _binary_search(data, key, lo, mid-1);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < size; i++){
		 random = rand() % 18 + 3;
		 int index = binary_search(list, random, size);
		 int check = 0;
		 for(int j = 0; j < size; j++){
			 if(random == list[j]){
				 cout << random << "		" << "is @[" << index << "]" << endl;
				 check = 1;
			 }
		 }
		 if(check == 0) cout << random << "		" << "is not @["  << index << "]" << endl;
	 }
