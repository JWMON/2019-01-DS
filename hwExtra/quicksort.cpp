...
int partition(int list[], int lo, int hi, int (*comp)(int a, int b)) {
	int x = list[hi];  // pivot
...
//
void _quickSort(int *list, int lo, int hi, int N, int (*comp)(int a, int b)) {
	if (lo < hi) 	{
...
void quickSort(int *a, int n, int (*comp)(int a, int b)) {

	_quickSort(a, 0, n - 1, n, comp);  // the last argument n is added only for DPRINT()
