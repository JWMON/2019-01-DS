...
void trim(heap p) {
	...
	int max = p->nodes[1];
	p->nodes[1] = p->nodes[p->N--];
	sink(p, 1);

	if (p->N + 1 == 1 / 4 * p->capacity)
		reserve(p, 1 / 2 * p->capacity);
...
void swap(heap p, int i, int j) {
	std::swap(p->nodes[i], p->nodes[j]);  // using std::swap()
}
...
void trimN(heap p, int count, bool heapOrdered) {
	...
	//void(*insertFunc)(heap h, int key) = heapOrdered ? trim : trimCBT;
