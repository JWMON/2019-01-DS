...
void heapprint(heap p) {
	...
	if (empty(p)) return;
	int count = 1;
	tree root = new TreeNode(p->nodes[count]);
	//cout << root->key << endl;
	...
	while (count < p->N) {
		count++;
		tree node = new TreeNode(p->nodes[count]);
		tree qf = que.front();

		if (qf->left == nullptr) {
			qf->left = node;
		}
		else if (qf->right == nullptr) {
			qf->right = node;
		}
		if(qf->left != nullptr && qf->right != nullptr)
			que.pop();
		que.push(node);
	}
	...
	//treeprint(new TreeNode(p->nodes));
