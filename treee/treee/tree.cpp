...
tree trimplus(tree node, int key) {
	...
	if (key < node->key)
		...
		if (height(node->left) <= height(node->right)) {
			tree tmp = succ(node);
			node->key = tmp->key;
			node->right = trimplus(node->right, tmp->key);
		}
		else {
			tree tmp = pred(node);
			node->key = tmp->key;
			node->left = trimplus(node->left, tmp->key);
		}
	}
...
void inorder(tree t, std::vector<tree>& v) {
	if (t == nullptr) return;
...
tree rebalanceTree(tree node) { 
	...
	vector<tree> v;
	inorder(node, v);
	tree a = buildAVL(v.data(), v.size());
	DPRINT(cout << "<rebalanceTree " << endl;);
	return a;
}
// builds AVL from an array of keys sorted
...
// builds AVL from an array of nodes sorted
